...
pub fn generate_command_file(options: TauriHelperOptions) {
    ...
    for member in workspace_members {
                    ...
                    if let Ok(ast) = parse_file(&content) {
                        for item in ast.items {
                            if let syn::Item::Fn(func) = item {
                                let mut is_tauri_command = false;
                                let mut is_auto_collect = false;

                                // Check for #[tauri::command] and #[auto_collect_command]
                                for attr in &func.attrs {
                                    if attr.path().is_ident("tauri")
                                ...
                                // Collect the function based on the options
                                if options.collect_all && is_tauri_command {
                                    // Collect all #[tauri::command] functions
                                    functions.push(func.sig.ident.to_string());
                                } else if is_tauri_command && is_auto_collect {
                                    // Collect only functions with both #[tauri::command] and #[auto_collect_command]
                                    functions.push(func.sig.ident.to_string());
                                }
                            }
                        }
        ...
        for func in functions {
            // Due to some Tauri shenanigans and some of my laziness and also because some options are only available in nightly.
